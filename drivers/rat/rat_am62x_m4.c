...
void address_trans_set_region(uint32_t rat_base_addr, uint16_t region_num,
        uint64_t system_addr, uint32_t local_addr,
...
void address_trans_init(address_trans_params *params)
{
    ...
    __ASSERT(translate_config.num_regions<ADDR_TRANSLATE_MAX_REGIONS, "Exceeding maximum number of regions");

    for(i=0; i<translate_config.num_regions; i++)
    {
        __ASSERT(translate_config.rat_base_addr!=0, "RAT base address cannot be 0");
        __ASSERT(translate_config.region_config!=NULL, "RAT region config cannot be NULL");
    
        /* enable regions setup by user */
        address_trans_set_region(
            translate_config.rat_base_addr,
            i,
            translate_config.region_config[i].system_addr,
            translate_config.region_config[i].local_addr,
            translate_config.region_config[i].size,
            1
            );
    }
}
...
void *rat_get_local_addr(uint64_t system_addr)
{
    ...
    __ASSERT(translate_config.num_regions<address_trans_MAX_REGIONS, "Exceeding maximum number of regions");
    ...
    for(regionId=0; regionId<translate_config.num_regions; regionId++)
    {
        ...
        size_mask = ( (uint32_t)( ((uint64_t)1 << translate_config.region_config[regionId].size) - 1) );
    ...
    if(found)
    {
        ...
        uint32_t offset = system_addr - translate_config.region_config[regionId].system_addr;
